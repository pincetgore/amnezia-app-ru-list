...
- [Как помочь проекту (Contributing)](#как-помочь-проекту-contributing)
...
Скрипт собирает CIDR-диапазоны через ASN, дополняет их IP-адресами из DNS и статическими правилами, агрегирует (убирает дубли и вложенные подсети) и формирует в виде `ip-list-<дата_время>.json` (в формате AmneziaVPN) и `cidrs-<дата_tiempo>.txt` (простой список префиксов).
...
1. Прогоняет **18 тестов** (`pytest`), включая:
   - 6 конфигурационных тестов (структура `config.yaml`, валидность доменов, агрегация сетей)
   - 12 unit-тестов для ASN и DNS резолверов (с мокированием API)
...
### Unit-тесты резолверов (12 тестов в `test_resolvers.py`)
...
1. **Резолвер ASN** (`resolvers/asn.py`):
   - ✅ Успешное парсинг ответов RIPE NCC API (включая фильтрацию IPv6).
...
**A:** Да! Специально для этого генерируется файл `cidrs-<дата_время>.txt` (по одному префиксу на строку). Его легко интегрировать в скрипты статической маршрутизации, `ipset` или настройки брандмауэра вашего роутера.
...
Выполните команду для запуска всех 18 тестов:
...
Для запуска только unit-тестов резолверов (12 тестов):
...
├── test_resolvers.py       # 12 unit-тестов для резолверов (с мокированием)
├── resolvers/
