...
### Исключение приложений (только для Anroid и Windows)
...
3. **Валидность доменов**: в списке доменов нет опечаток (например, случайно скопированного префикса `http://`).
...
1. Прогоняет тесты (`pytest`), проверяя структуру `config.yaml` и логику склейки.
