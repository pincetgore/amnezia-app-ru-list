...
Скрипт собирает CIDR-диапазоны через ASN, дополняет их IP-адресами из DNS и статическими правилами, агрегирует (убирает дубли и вложенные подсети) и формирует в виде `ip-list-<дата_время>.json` (в формате AmneziaVPN) и `cidrs-<дата_время>.txt` (простой список префиксов).
Готовые файлы обновляются еженедельно и автоматически публикуются на странице **Releases**.
...
2. Скачайте нужный файл из последнего релиза:
   * `ip-list-<дата_время>.json` — для импорта в приложение AmneziaVPN.
   * `cidrs-<дата_время>.txt` — простой текстовый список сетей в формате CIDR (по одному префиксу на строку). Подходит для настройки маршрутизации в других VPN-клиентах (v2ray, sing-box, Xray), сторонних утилитах, брандмауэрах (например, iptables) или на домашних роутерах (OpenWrt, Keenetic и др.).
