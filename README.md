...
1. Прогоняет **24 теста** (`pytest`), включая:
   - 6 конфигурационных тестов (структура `config.yaml`, валидность доменов, агрегация сетей)
   - 18 unit-тестов для ASN и DNS резолверов (с мокированием API)
...
### Unit-тесты резолверов (18 тестов в `test_resolvers.py`)
...
Выполните команду для запуска всех 24 тестов:
...
Для запуска только unit-тестов резолверов (18 тестов):
...
- **resolvers/asn.py** — получает IPv4-префиксы от RIPE (3 retry попытки, exponential backoff). Fallback на bgp.he.net с парсингом HTML через BeautifulSoup.
...
- **ASN ошибки**: Если RIPE API недоступен или вернул пустые данные, используется fallback на bgp.he.net (через BeautifulSoup). Если оба источника недоступны, ASN пропускается.
